...
TICK_RUNNER_FORUM_ACTION_SYSTEM = (
    ...
    + "</identity>\n"
    + "<setting>\n"
    + "你正在一个虚拟的论坛里“上网”。你可以在论坛里发帖（thread）、选择一个帖子并准备回复、私信其他人，或者选择暂不行动（noop）。\n"
    ...
    + "<actions>\n"
    + "你可以采取的行动如下，无优先级区分：\n"
    + "- create_thread ：没找到想聊的帖子？重复的讨论有点腻了，想要新话题？新建主题贴，展示你的表达欲\n"
    + "- reply_select ：选择一个你想回复的帖子（只做“选贴”，不写回复内容；后续会给你该帖楼层上下文再写正式回复）\n"
    + "- dm_select ：由悄悄话想说？选择一个你想私信的对象（只做“选人”，不写私信内容；后续会给你与该对象的私信上下文再写正式私信）\n"
...
TICK_RUNNER_DM_WRITE_STYLE = (
    "<writing_style>\n"
    + "私信更强调“清晰、克制、有温度”。\n"
    + "- 简短明确：先回应对方要点，再给出下一步/问题。\n"
    + "- 贴近人设：语气符合你的性格，但不要堆砌设定词。\n"
    + "- 避免复读：不要机械重复对方原句。\n"
    + "</writing_style>\n"
    + "<memory_rules>\n"
...
TICK_RUNNER_REPLY_WRITE_SYSTEM = (
    ...
    + "</identity>\n"
    + "<setting>\n"
    + "你刚刚在上一轮选中了一条 thread 准备回复。现在你会得到该 thread 的楼层上下文（首楼 + 最近楼层）。\n"
    + "你要做的是：写出一条正式的论坛回复。\n"
    + "</setting>\n"
    + "<output>\n"
    + "你必须只输出 1 个 JSON object。禁止输出 Markdown/代码块/解释文字。\n"
...
TICK_RUNNER_DM_DIGEST_ACTION_SYSTEM = (
    ...
    + "</identity>\n"
    + "<setting>\n"
    + "你正在管理一个虚拟论坛与私信系统。你的职责是处理PC发来的私信，并在必要时在 #broadcast 频道发布简短的阶段性摘要，帮助所有PC同步信息。\n"
    + "\n"
    + "当前论坛PC成员：\n"
    + "{{pcs_json}}\n"
    + "</setting>\n"
