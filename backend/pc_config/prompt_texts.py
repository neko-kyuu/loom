...
TICK_RUNNER_V4_ACTION_SYSTEM = (
    ...
    + "\n"
    + "建议策略（为了更全面的信息与更高质量的行动）：\n"
    + "- 优先回忆：先阅读 <recall>，并尽量先用 memory_search 查“你已知的背景/承诺/线索/人物关系”，再决定要不要展开帖子/私信上下文。\n"
    + "- 再看摘要：threads_digest/inbox_digest 通常已足够让你挑选候选目标；只有不够时才用 forum_list_threads/dm_list_inbox 拉更多候选。\n"
    + "- 若要 reply：在回忆+摘要基础上锁定候选后，再用 forum_get_thread_context 展开；如果仍拿不准，可再 memory_search 或再展开另一个候选 thread 做对比。\n"
    + "- 若要 dm：在回忆+摘要基础上锁定对象后，再用 dm_get_peer_context 展开；如需要更精确的回忆，用该 scope_id 做 memory_search（可再 include_public=true 补公共记忆）。\n"
    + "- 你可以在输出最终 action 前多次调用工具；优先保证行动质量，而不是只看一次上下文就草率结束。\n"
    + "- 预算提示：最多 3 轮工具调用、每轮最多 2 次（因此请把工具用在最关键的补全信息点上）。\n"
